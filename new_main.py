...
if __name__ == '__main__':
    ...
    AudioData = TorchAudioDatasetWrapper(50, 25, "raw_audio", (0.6, 0.1, 0.3))
    ...
    testing_load = torch.utils.data.DataLoader(AudioData.testing_set, 1, shuffle=True, num_workers=0, pin_memory=False, drop_last=True)

    data, data_shift1 = next(iter(testing_load))
    data_reshaped = torch.squeeze(torch.reshape(data, (1, 1, 50)))
    ...
    plt.plot(x, torch.squeeze(prediction).cpu().detach().numpy())
    plt.show()